...
const useGetSavingsAccounts = (userEmail: string) => {
  return useQuery<Account[]>({
    queryKey: [userEmail, "accounts"],
    staleTime: 24 * 60 * 60 * 1000, // 24 hours
    queryFn: async () => {
      const response = await fetch(apiUrlByUser[userEmail]);

      return response.json();
    },
  });
};
const useUploadSavingsSnapshot = (userEmail: string) => {
  return useMutation({
